...
router.put('/schedules/:id', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const result = await client.query(`UPDATE schedules SET route_id = $1, bus_id = $2, departure_time = $3, arrival_time = $4, days_of_week = $5, price_multiplier = $6, status = $7 WHERE id = $8 RETURNING *`, [fields.route_id, fields.bus_id, fields.departure_time, fields.arrival_time, fields.days_of_week, fields.price_multiplier, fields.status, id]);
...
router.delete('/schedules/:id', requireAdmin, async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        const reservationsCheck = await client.query('SELECT id FROM reservations WHERE schedule_id = $1 AND status IN ($2, $3) LIMIT 1', [id, 'pending', 'confirmed']);
