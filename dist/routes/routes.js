...
router.post('/', checkAdmin, async (req, res) => {
    ...
    try {
        const existingRoute = await pool.query('SELECT id FROM routes WHERE LOWER(origin) = LOWER($1) AND LOWER(destination) = LOWER($2)', [origin, destination]);
        if (existingRoute.rows.length > 0) {
            return res.status(400).json({ error: 'Ya existe una ruta con el mismo origen y destino' });
        }
        const result = await pool.query('INSERT INTO routes (origin, destination, distance_km, base_price) VALUES ($1, $2, $3, $4) RETURNING *', [origin, destination, distance_km, base_price]);
...
router.put('/:id', checkAdmin, async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query('UPDATE routes SET origin = $1, destination = $2, distance_km = $3, base_price = $4 WHERE id = $5 RETURNING *', [fields.origin, fields.destination, fields.distance_km, fields.base_price, id]);
