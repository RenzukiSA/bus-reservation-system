...
router.get('/schedules', validate('query', TripQuerySchema), async (req, res) => {
    ...
    try {
        ...
        const schedulesWithAvailability = await Promise.all(schedulesResult.rows.map(async (schedule) => {
            const reservationsResult = await pool.query(`SELECT reservation_type, seats_reserved FROM reservations WHERE schedule_id = $1 AND reservation_date = $2 AND status IN ('pending', 'confirmed')`, [schedule.schedule_id, date]);
            let reservedSeatIds = [];
...
router.get('/seats/:schedule_id', async (req, res) => {
    ...
    try {
        ...
        const reservationsRes = await pool.query(`SELECT reservation_type, seats_reserved FROM reservations WHERE schedule_id = $1 AND reservation_date = $2 AND status IN ('pending', 'confirmed')`, [schedule_id, date]);
...
router.post('/', checkAdmin, async (req, res) => {
    ...
    try {
        ...
        const result = await client.query('INSERT INTO buses (bus_number, type, capacity, status) VALUES ($1, $2, $3, $4) RETURNING *', [bus_number, type, capacity, status]);
...
router.put('/:id', checkAdmin, async (req, res) => {
    ...
    try {
        ...
        const result = await client.query('UPDATE buses SET bus_number = $1, type = $2, capacity = $3, status = $4 WHERE id = $5 RETURNING *', [fields.bus_number, fields.type, fields.capacity, fields.status, id]);
        await client.query('COMMIT');
        res.json(result.rows[0]);
    }
    catch (err) {
...
router.delete('/:id', checkAdmin, async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        const reservationCheck = await client.query(`SELECT r.id FROM reservations r
...
async function createSeatsForBus(client, busId, capacity) {
    ...
    for (let i = 1; i <= capacity; i++) {
        ...
        await client.query('INSERT INTO seats (bus_id, seat_number, seat_type, price_modifier) VALUES ($1, $2, $3, $4)', [busId, String(i), seatType, priceModifier]);
    }
}
