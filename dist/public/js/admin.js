...
function loadSection(sectionId) {
    ...
    switch (sectionId) {
        ...
        case 'reservations':
            ...
            break;
        case 'routes':
            loadGenericAdminSection('routes');
            break;
        case 'buses':
            loadGenericAdminSection('buses');
            break;
        case 'schedules':
            loadGenericAdminSection('schedules');
            break;
    }
}
// --- DASHBOARD SECTION --- 
...
async function updateReservationStatus(id, status) {
    ...
    if (!confirm(`¿Estás seguro de que quieres ${action} esta reservación?`))
        return;
    try {
...
async function loadGenericAdminSection(sectionKey) {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(`No se pudieron cargar los ${config.plural}.`);
        ...
        if (field.type === 'select') {
            ...
            if (field.source) { // Dynamically load options
                try {
                    ...
                    if (field.source === 'routes')
                        options = sourceItems.reduce((acc, i) => ({ ...acc, [i.id]: `${i.origin} → ${i.destination}` }), {});
                    if (field.source === 'buses')
                        options = sourceItems.reduce((acc, i) => ({ ...acc, [i.id]: `${i.bus_number} (${i.capacity} asientos)` }), {});
                }
                catch { /* ignore */ }
            }
...
async function handleDeleteItem(sectionKey, itemId) {
    const config = SECTIONS_CONFIG[sectionKey];
    if (!confirm(`¿Estás seguro de que quieres eliminar esta ${config.singular.toLowerCase()}?`))
        return;
    try {
        const response = await fetch(`${config.endpoint}/${itemId}`, {
            method: 'DELETE',
